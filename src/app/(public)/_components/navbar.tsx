...
export function Navbar() {
  ...
  return (
    <header
      id="navbar"
      className="fixed top-0 left-0 right-0 z-50 w-full bg-background/60 backdrop-blur-md"
    >
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {NAV_LINKS.map(({ label, href }) => (
            <Link
              key={label}
              href={href}
              className="rounded-md px-3 py-2 text-sm text-black transition-colors hover:bg-muted hover:text-foreground"
            >
              {label}
            </Link>
          ))}
